...
/*   Updated: 2025/09/07 17:06:58 by ekamar           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	check_valid_args(int ac, char **av)
{
	...
	if (check_arg_content(av[1]) || ft_atoi(av[1]) <= 0
		|| exceeds_int_max(av[1]))
		return (print_error("Invalid philosophers\n"));
	if (check_arg_content(av[2]) || ft_atoi(av[2]) <= 0
		|| exceeds_int_max(av[2]))
		return (print_error("Invalid time_to_die\n"));
	if (check_arg_content(av[3]) || ft_atoi(av[3]) <= 0
		|| exceeds_int_max(av[3]))
		return (print_error("Invalid time_to_eat\n"));
	if (check_arg_content(av[4]) || ft_atoi(av[4]) <= 0
		|| exceeds_int_max(av[4]))
		return (print_error("Invalid time_to_sleep\n"));
	if (ac == 6 && (check_arg_content(av[5]) || ft_atoi(av[5]) <= 0
			|| exceeds_int_max(av[5])))
